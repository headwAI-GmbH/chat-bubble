...
-- Note: OpenAI provider settings are intentionally configured via backend API
-- below to ensure runtime state is updated (not only DB state).
...
if ADMIN_BEARER_TOKEN=$(curl -s --fail-with-body "$API_URL/api/v1/auths/signin" \
  ...
  echo "Verifying API key feature is enabled..."
  ENABLE_API_KEYS_CHECK=$(curl -s --fail-with-body "$API_URL/api/v1/auths/admin/config" \
    -H "authorization: Bearer $ADMIN_BEARER_TOKEN" 2>/dev/null | jq -r '.ENABLE_API_KEYS // false' 2>/dev/null)

  if [ "$ENABLE_API_KEYS_CHECK" = "true" ]; then
    echo "  OK: ENABLE_API_KEYS is true"
  else
    echo "ERROR: ENABLE_API_KEYS is not true"
    exit 1
  fi
  ...
  echo "Verifying OpenAI config via API..."
  OPENAI_CONFIG_CHECK=$(curl -s --fail-with-body "$API_URL/openai/config" \
    -H "authorization: Bearer $ADMIN_BEARER_TOKEN" 2>/dev/null | jq -r '.OPENAI_API_BASE_URLS[]?' 2>/dev/null | grep -c "mock-llm" || echo "0")
  
  if [ "$OPENAI_CONFIG_CHECK" -gt 0 ]; then
    echo "  OK: mock-llm endpoint found in OpenAI config"
  else
    echo "ERROR: mock-llm endpoint not found in OpenAI config"
    exit 1
  fi
